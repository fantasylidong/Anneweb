-- SourceBans + L4D2 Stats & Plugin Schema
-- MySQL 5.7 compatible definitions kept for backward compatibility.
...
CREATE TABLE `chat_log` (
  ...
  PRIMARY KEY (`id`),
  KEY `chat_log_date_index` (`date`),
  KEY `chat_log_steamid_index` (`steamid`),
  KEY `chat_log_server_index` (`server`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4;
...
CREATE TABLE `sb_admins` (
  ...
  `expires_at` timestamp NULL DEFAULT NULL COMMENT '管理员有效期，为空表示永久',
  PRIMARY KEY (`aid`),
...
DROP TABLE IF EXISTS `sb_admin_renewals`;
CREATE TABLE `sb_admin_renewals` (
  `id` bigint(20) unsigned NOT NULL AUTO_INCREMENT,
  `admin_id` int(10) unsigned NOT NULL,
  `previous_expires_at` timestamp NULL DEFAULT NULL,
  `new_expires_at` timestamp NOT NULL,
  `extended_by` bigint(20) unsigned DEFAULT NULL COMMENT '操作源，例如后台用户 ID',
